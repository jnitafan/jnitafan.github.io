...
import { Html, OrbitControls, Outlines, GizmoViewport, GizmoHelper, Box, CycleRaycast } from '@react-three/drei'
...
const Cube = (props) => {
	const [ref] = useBox(() => ({ mass: 1, args: [props.scale, props.scale, props.scale], ...props }))
...
const CubeCanvas = ( tech ) => {
  ...
  return (
		<div ref={ref} className='h-full'>
			<Canvas frameloop={inView ? 'always' : 'never'} dpr={[1, 2]} shadows camera={{ position: [-5, 5, 5], fov: 50 }}>
				...
				>
					...
					<Physics>
						...
						{tech.cubes.map((technology, index) => (
							<Cube position={[getRandomRange(-7.5, 7.5), 5 + (index * 2), getRandomRange(-7.5, 7.5)]} scale={technology.proficiency} img={technology.icon} name={technology.name} key={technology.name} />
						))}
					</Physics>
					<CycleRaycast
						scroll={false}
